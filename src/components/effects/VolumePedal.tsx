...
const VolumePedal = ({ disabled }: Props) => {
  ...
  const handleToggle = () => {
    if (isActive) volume.deactivate()
    else volume.activate()

    setIsActive((value) => !value)
  }

  return (
    <div className='flex flex-col w-48 max-w-[16rem] h-[500px] p-6 rounded-xl items-center justify-between space-y-8 shadow-pedal bg-white'>
